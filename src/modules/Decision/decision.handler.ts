...
export class DecisionHandler{
    constructor(private eventBus: EventBus, private decisionEngine: DecisionEngine){
    ...
    register(){
        this.eventBus.subscribe(EventType.AI_ANALYZED, async(event)=>{
            ...
            await this.eventBus.publish({type: EventType.DECISION_MADE, timestamp: new Date(), payload:{claim,fraud,ai, decision}})
        })
    }
}